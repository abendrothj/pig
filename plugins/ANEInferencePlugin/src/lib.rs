...
unsafe extern "C" fn run(input: *const PluginInput) -> PluginOutput {
    ...
    // If input looks like a JSON config, use it; otherwise return capabilities
    let result = if input_text.trim().starts_with('{') {
        // Parse config and return inference estimate
        let tokens: usize = serde_json::from_str::<serde_json::Value>(&input_text)
            .ok()
            .and_then(|v| v.get("tokens").and_then(|t| t.as_u64()))
            .unwrap_or(1) as usize;

        format!(
            r#"{{"plugin":"ANEInferencePlugin","available":{},"chip":"{}","tops":{},"power_w":{},"estimated_latency_ms":{:.1}}}"#,
            caps.available, caps.chip_model, caps.peak_tops, caps.power_draw_w,
            ANEInferencePlugin::estimate_latency_ms(tokens)
        )
    } else {
        format!(
...
unsafe extern "C" fn get_metadata() -> PluginMetadata {
    static NAME: &[u8] = b"ANEInferencePlugin\0";
    static VERSION: &[u8] = b"1.0.0\0";
    static DESCRIPTION: &[u8] = b"Apple Neural Engine inference plugin for M-series chips\0";
    static AUTHOR: &[u8] = b"LAO Team\0";
    static TAGS: &[u8] = b"[\"inference\", \"apple-silicon\", \"neural-engine\", \"ane\"]\0";
    static CAPABILITIES: &[u8] = b"[{\"name\":\"ane-inference\",\"description\":\"Run inference on Apple Neural Engine\",\"input_type\":\"Text\",\"output_type\":\"Json\"}]\0";
